...
class Torus:
    ...
    # The following are old versions of pred, succ, and conf
    def predecessor_version_one(self, (u, v)):
        has_edge = self.graph.has_edge(u, v)
        assert has_edge, "The edge from %r" % u + " to %r" % v + " does not exist."
        ...
        if u == 0:
            if v == self.n:
                predecessor_edge = ((self.size - self.n), u)
            elif v == 1:
                predecessor_edge = ((self.n - 1), u)
        elif u == (self.n - 1):
            if v == 0:
                predecessor_edge = ((u - 1), u)
            elif v == (u + self.n):
                predecessor_edge = ((self.size - 1), u)
            elif v == (self.size - 1):
                predecessor_edge = ((u + self.n), u)
        elif u == (self.size - self.n):
            if v == 0:
                predecessor_edge = ((u - self.n), u)
            elif v == (self.size - 1):
                predecessor_edge = ((u + 1), u)
            ...
            if v == u + 1:
                predecessor_edge = ((u - 1), u)
            elif v == ((self.size - self.n) + u):
                predecessor_edge = ((u + self.n), u)
            elif v == (u + self.n):
                predecessor_edge = (((self.size - self.n) + u), u)
        elif (self.n - 1) < u < (self.size - self.n) and (u % self.n) == 0:
            if v == (u + (self.n - 1)):
                predecessor_edge = ((u + 1), u)
            elif v == (u + 1):
                predecessor_edge = ((u + (self.n - 1)), u)
            elif v == (u + self.n):
                predecessor_edge = ((u - self.n), u)
        elif (self.n - 1) < u < (self.size - self.n) and ((u + 1) % self.n) == 0:
            if v == (u - 1):
                predecessor_edge = ((u - (self.n - 1)), u)
            elif v == (u - (self.n - 1)):
                predecessor_edge = ((u - 1), u)
                ...
                predecessor_edge = ((u + self.n), u)
        elif (self.size - self.n) < u < (self.size - 1):
            if v == (u - 1):
                predecessor_edge = ((u + 1), u)
            elif v == (u + 1):
                predecessor_edge = ((u - 1), u)
            ...
            if v == (u + 1):
                ...
                predecessor_edge = ((u + self.n), u)
        check = False
        if 'predecessor_edge' in locals():
            check = True
        assert check, "The preceding edge from (%r," % u + "%r) could not be found." % v
        check = self.graph.has_edge(predecessor_edge[0], predecessor_edge[1])
        assert check, "A preceding edge was found but does not exist in the graph."
        del check
        return predecessor_edge

    def successor_version_one(self, (u, v)):
        has_edge = self.graph.has_edge(u, v)
        assert has_edge, "The edge from %r" % u + " to %r" % v + " does not exist."
        del has_edge

        if u == 0:
            if v == self.n and self.n > 2:
                successor_edge = (v, (v + self.n))
            elif v == self.n and self.n == 2:
                successor_edge = (v, u)
            elif v == 1 and self.n > 2:
                successor_edge = (v, (v + 1))
            elif v == 1 and self.n == 2:
                successor_edge = (v, u)
        elif u == self.n - 1:
            if v == 0:
                successor_edge = (v, (v + 1))
            elif v == (u + self.n) and self.n > 2:
                successor_edge = (v, (v + self.n))
            elif v == (self.size - 1):
                successor_edge = (v, (v - self.n))
        elif u == (self.size - self.n):
            if v == 0:
                ...
                successor_edge = (v, (v - 1))
            elif v == (u + 1) and self.n > 2:
                successor_edge = (v, (v + 1))
        elif u == (self.size - 1):
            if v == (self.size - self.n) and self.n > 2:
                successor_edge = (v, (v + 1))
            elif v == (u - 1) and self.n > 2:
                successor_edge = (v, (v - 1))
            elif v == (u - 1) and self.n == 2:
                successor_edge = (v, (self.size - 1))
            elif v == (u - self.n) and self.n > 2:
                successor_edge = (v, (v - self.n))
            elif v == (u - self.n) and self.n == 2:
                successor_edge = (v, (self.size - 1))
            elif v == (self.n - 1) and self.n > 2:
                successor_edge = (v(v + self.n))
        elif 0 < u < (self.n - 1):
            if v == (self.n - 1):
                successor_edge = (v, 0)
            ...
            if v == (u - 1):
                ...
                successor_edge = (v, (v + 1))
            elif v == (u + self.n) and v != (self.size - 1):
                successor_edge = (v, (v + self.n))
            elif v == (u + self.n) and v == (self.size - 1):
                successor_edge = (v, (self.n - 1))
            elif v == (self.n - 1):
                successor_edge = (v, (self.size - 1))
            elif v == (u - self.n):
                successor_edge = (v, (v - self.n))
        elif (self.size - self.n) < u < (self.size - 1):
            if v == (u - 1) and v != (self.size - self.n):
                successor_edge = (v, (v - 1))
            elif v == (u - 1) and v == (self.size - self.n):
                successor_edge = (v, (self.size - 1))
            elif v == (u + 1) and v != (self.size - 1):
                successor_edge = (v, (v + 1))
            elif v == (u + 1) and v == (self.size - 1):
                successor_edge = (v, (self.size - self.n))
            elif v == (u % self.n):
                successor_edge = (v, (v + self.n))
            elif v == (u - self.n):
                successor_edge = (v, (v - self.n))
        else:
            if v == (u + 1) and (v + 1) % self.n != 0:
                successor_edge = (v, (v + 1))
            elif v == (u + 1) and (v + 1) % self.n == 0:
                successor_edge = (v, (v - (self.n - 1)))
            elif v == (u - 1) and (v % self.n) != 0:
                successor_edge = (v, (v - 1))
            elif v == (u - 1) and (v % self.n) == 0:
                successor_edge = (v, (v + (self.n - 1)))
            elif v == (u + self.n) and v < (self.size - self.n):
                successor_edge = (v, (v + self.n))
            elif v == (u + self.n) and v > (self.size - self.n):
                successor_edge = (v, (v % self.n))
            elif v == (u - self.n) and v > self.n:
                successor_edge = (v, (v - self.n))
            elif v == (u - self.n) and v < self.n:
                successor_edge = (v, ((self.size - self.n) + v))
        check = False
        if 'successor_edge' in locals():
            check = True
        assert check, "The succeeding edge from (%r," % u + "%r) could not be found." % v
        check = self.graph.has_edge(successor_edge[0], successor_edge[1])
        assert check, "A successor node was found but does not exit in the graph."
        del check

        return successor_edge

    def conflict_version_one(self, (u, v)):
        has_edge = self.graph.has_edge(u, v)
        assert has_edge, "The edge from %r" % u + " to %r" % v + " does not exist."
        del has_edge
        # convert u and v into position tuples (work from there) ex: (x, y) (x', y')

        if u == 0:
            if v == self.n:
                conflicting_edge = ((v + 1), v)
            elif v == 1:
                conflicting_edge = ((v + self.n), v)
        elif u == (self.n - 1):
            if v == 0:
                conflicting_edge = ((self.size - self.n), v)
            elif v == (self.size - 1):
                conflicting_edge = ((self.size - self.n), v)
            elif v == (u + self.n):
                conflicting_edge = ((v + 1), v)
            ...
            if v == (u + 1):
                if (v % 2) == 0:
                    conflicting_edge = (((self.size - self.n) + v), v)
                else:
                    conflicting_edge = ((v + self.n), v)
            elif v == ((self.size - self.n) + v):
                if (self.n % 2) == 0:
                    conflicting_edge = (v + 1)
                else:
                    conflicting_edge = ((v - 1), v)
            elif v == (u + self.n):
                conflicting_edge = ((v + 1), v)
            ...
            if v == (u + (self.n - 1)):
                ...
                if self.graph.has_edge(v, (v + 1)):
                    conflicting_edge = ((v + (self.n - 1)), v)
                else:
                    conflicting_edge = ((v + 1), v)
        elif (self.n - 1) < u < (self.size - self.n) and ((u + 1) % self.n) == 0:
            if v == (u - 1):
            ...
            if v == (u + 1):
                if self.graph.has_edge(v, (v - self.n)):
                    conflicting_edge = ((v % self.n), v)
                else:
                    conflicting_edge = ((v - self.n), v)
            elif v == (u - 1):
            ...
            if v == (u - 1):
                if self.graph.has_edge((v - self.n), v):
                    conflicting_edge = ((v - self.n), v)
                else:
                    conflicting_edge = ((v + self.n), v)
            elif v == (u + 1):
                ...
                if self.graph.has_edge((v - 1), v):
                    ...
                    conflicting_edge = ((v + 1), v)
        check = False
        if 'conflicting_edge' in locals():
            check = True
        assert check, "The conflicting edge to (%r," % u + "%r) could not be found." % v

        check = self.graph.has_edge(conflicting_edge[0], conflicting_edge[1])
        assert check, "A conflicting edge was found but does not exist."
        del check
